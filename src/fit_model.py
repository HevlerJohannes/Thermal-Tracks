...
# Import optimized gp modules (updated imports)
from build_model import calculate_optimal_batch_size, building_exactgp_model_batched_optimized

def compute_batch_mll_parallel(model_data):
    """Helper function for parallel MLL computation"""
...
def train_model_batched_optimized(tpptr_df_input, parameters, null_dataset=False):
    """
    Optimized training for batched GP models with parallel processing
    
    Args:
        tpptr_df_input: Input dataframe
        parameters: Dictionary of parameters (now includes parallel processing options)
        null_dataset: Whether this is null dataset training
    
    Returns:
        train_results_dict: Dictionary with training results
    """
    ...
    sample_cond = tpptr_df[tpptr_df['uniqueID'] == sample_protein]['condition'].unique()[0]
    sample_df = tpptr_df[(tpptr_df['uniqueID'] == sample_protein) & 
    ...
    model_list, n_cond, n_prot, n_models = building_exactgp_model_batched_optimized(
        tpptr_df, parameters, proteins2test, conds, 
        scale_kernel, lengthscale_prior, 
        lengthscale_minconstraint, lengthscale_mult, 
        mean, batch_size=batch_size, device=device, 
        null_dataset=null_dataset,
    ...
    for i in range(training_iterations):
        ...
        if is_notebook():
            if i % max(1, training_iterations // 20) == 0 or i == training_iterations - 1:
                if null_dataset == False:
            ...
            for task_idx in range(num_tasks_in_model):
                ...
                if isinstance(model.covar_module, gpytorch.kernels.ScaleKernel):
                    if num_tasks_in_model > 1:
                        lengthscale = model.covar_module.base_kernel.lengthscale[task_idx].item()
                ...
                if task_idx < len(meta_chunk):
                    ...
                    training_info.append([
                        i + 1, normalized_loss.item(), meta['global_index'] + 1,
                        lengthscale, outputscale, noise,
                        meta['protein'], meta['condition']
                    ])
    ...
    # Update checklist
    if is_notebook():
        live_update_message = f"Training complete after {len(LossValues)} iterations ({train_time:.1f}s)"
        update_checklist(tasks, live_update={"task": "1. Build and fit full model",
                                            "subtask": "Train models",
                                            "message": live_update_message}) 
        if null_dataset == False:
    ...
    if use_parallel and len(model_list) > 1:
        ...
        with ThreadPoolExecutor(max_workers=n_workers) as executor:
            mll_results_nested = list(executor.map(compute_batch_mll_parallel, model_list))
    ...
    if training_info:
        training_info_df = pd.DataFrame(
            training_info,
            columns=['iteration', 'loss', 'model', 'lengthscale', 
                    'outputscale', 'noise', 'protein', 'condition']
        )
    ...
    if null_dataset == False:
        plt.savefig(f'{output_path}/loss_full_model_gp_{control}_{pert}.pdf', 
                   bbox_inches='tight', dpi=300)
    else:
        plt.savefig(f'{output_path}/loss_null_model_gp_{control}_{pert}.pdf',
                   bbox_inches='tight', dpi=300)
    plt.close()
